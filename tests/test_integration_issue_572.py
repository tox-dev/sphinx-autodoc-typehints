...
from sphinx_autodoc_typehints import _get_type_hint
...
def test_get_type_hint_uses_annotationlib_on_name_error() -> None:
    ...
    with (
        patch("sphinx_autodoc_typehints.get_type_hints", side_effect=NameError("name 'Foo' is not defined")),
...
def test_get_type_hint_falls_back_to_dunder_annotations_before_314() -> None:
    """_get_type_hint falls back to __annotations__ on pre-3.14 NameError."""

    def dummy(x: int) -> str: ...

    with patch("sphinx_autodoc_typehints.get_type_hints", side_effect=NameError("name 'Foo' is not defined")):
        result = _get_type_hint([], "dummy", dummy, {})
